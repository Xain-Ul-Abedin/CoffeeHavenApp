...
![Logo Placeholder](https://via.placeholder.com/800x200.png?text=Coffee+Haven+Enterprise)
...
To ensure strict boundary controls, Role selection occurs *before* database querying.
```mermaid
sequenceDiagram
    participant User
...
- **Admin Account**: 
  ...
  - *Note: You can initialize additional admins using the `ADMIN2026` secret switch during registration phase.*
